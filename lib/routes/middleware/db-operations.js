...
const regionRouteOps = {
  async getRegionId(req, res) {
    const getRegionIdOp = async (req, res, cb = null) => await getRegion(req.params.id)
      .then(async regions => {
        if (cb !== null){
          await cb(regions);
        }
...
const vendorRouteOps = {
  getVendors: async (req, res) => {
    ...
    const getVendorsOp = async (req, res, cb = null) => await getVendors(req.params.regionID)
      .then(async (vendors) => {
        if (cb !== null) {
          await cb(vendors);
        }
  ...
  getVendorById: (req, res) => {
    const getVendorByIdOp = async (req, res, cb = null) => await getVendor(req.params.regionID, req.params.vendorID)
      .then(async (vendor) => {
        if (cb !== null) {
          await cb(vendor);
        }
 ...
 regionID: req.params.regionID, vendorID: req.params.vendorID, type: req.body.type, locationID: req.body.locationID, amount: req.body.amount 
})
    .then(update => res.status(200).json(update))
    .catch(err => res.status(500).send(err)),

  putRegionIdVendorIdComments: async (req, res) => updateVendorPush({
 regionID: req.params.regionID, vendorID: req.params.vendorID, field: req.body.field, payload: req.body.payload 
})
    .then(update => res.status(200).json(update))
    .catch(err => res.status(500).send(err)),
};
