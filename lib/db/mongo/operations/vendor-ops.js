...
module.exports = {
  ...
  updateVendorPush: function(params) {
    ...
    .then( async res => {
      await redisClient.hdelAsync('vendor', `q::method::GET::path::${regionID}`);
      await redisClient.hdelAsync('vendor', `q::method::GET::path::${regionID}/${vendorID}`);
      return res
    })
    .catch( err => {
  ...
  updateLocationAccuracy: function(params) {
    if (params.regionID === undefined || params.vendorID === undefined || params.type === undefined || params.locationID === undefined || params.amount === undefined) {
      console.error(new Error('Missing params for updateLocationAccuracy'));
      return Promise.reject(new Error('Missing params for updateLocationAccuracy'));
    }
    ...
    if (amount === 1 || amount === -1) {
      ...
      .then( async res => {
        await redisClient.hdelAsync('vendor', `q::method::GET::path::${regionID}/${vendorID}`);
        const resy = await redisClient.hgetAsync('vendor', `q::method::GET::path::${regionID}/${vendorID}`)
