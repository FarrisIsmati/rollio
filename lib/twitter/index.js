...
const twitter = {
  streamClient : async function(cb, vendorList, tstIDs='1053649707493404678') {
    let userIDs;
    if (process.env.NODE_ENV === 'TEST') {
      userIDs = tstIDs;
    } else {
      userIDs = vendorList
      console.log('Recieved VendorIDs, length: ' + userIDs.length);
    }
    ...
    //Need an exponential backoff reconnect strategy
    stream.on('error', err => {
      throw err;
    })
    return stream;
