  ...
  _gatherFilePath(filePath) {
    ...
    if (regexMatch) {
      ...
      const compiledTemplate = (context) => {
        try {
          const mjmlTemplate = this._mjmlWrapper(template(context), context);
...
// Setup templates for gatherFilePath
paths.forEach((filePath) => templating._gatherFilePath(filePath));
